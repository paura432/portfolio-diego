...
export default function ConciertoEventPage() {
  const { language } = useTheme();
  const params = useParams();
  const id = params.id as string;
  const t = getTranslations(language);
  
  const event = getPhotoEvent('conciertos', id, language);

  if (!event) {
    return (
      <div className="max-w-7xl mx-auto px-4 md:px-6 py-12 text-center">
        <h1 className="text-2xl mb-4">Evento no encontrado</h1>
        <Link href="/photography/conciertos" className="text-primary-600 dark:text-primary-400">
          Volver a conciertos
        </Link>
      </div>
    );
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-10 md:py-12">
      ...
      >
        ...
        {t.photography.backTo} {t.photography.conciertos}
      </Link>
